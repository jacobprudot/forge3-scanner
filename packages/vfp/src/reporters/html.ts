...
export function reportHtml(report: UniversalScanReport): string {
  const { detection } = report
  const confPct = Math.round(detection.primary.confidence * 100)
  const techName = detection.primary.technology.toUpperCase()
  const isDeep = isDeepAnalysis(report.analysis)
...
<head>
  ...
  <title>Forge3 Scanner — ${techName} Report</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .notice { background: #1a1700; border: 1px solid #ca8a04; border-radius: 8px; padding: 1rem; color: #eab308; }
...
<body>
  ...
  <div class="section">
    ...
    <div class="stat-grid">
      <div class="stat">
        <div class="label">Primary Technology</div>
        <div class="value">${techName}</div>
        <div class="label">${confPct}% confidence</div>
      </div>
      <div class="stat">
        <div class="label">Files Detected</div>
        <div class="value">${n(detection.primary.fileCount)}</div>
        <div class="label">${detection.primary.evidence.length} evidence points</div>
      </div>
      <div class="stat">
        <div class="label">Analysis Depth</div>
        <div class="value">${isDeep ? "Deep" : "Basic"}</div>
        <div class="label">${isDeep ? "Full analysis available" : "Basic inventory only"}</div>
      </div>
    </div>
    ${detection.secondary.length > 0 ? `
    <p style="margin-top:1rem;color:#888;font-size:0.85rem">Also detected: ${detection.secondary.map((s) => `${s.technology} (${s.fileCount} files)`).join(", ")}</p>
    ` : ""}
  </div>

${isDeep ? renderDeepHtml(report.analysis as ScanReport) : renderBasicHtml(report.analysis as BasicAnalysis)}

${renderProfileHtml(report.profile)}
...
function renderDeepHtml(report: ScanReport): string {
...
function renderBasicHtml(analysis: BasicAnalysis): string {
  const inv = analysis.inventory
  const topExtensions = inv.fileCounts.slice(0, 20)

  return `
  <div class="section">
    <h2>📋 Inventory (Basic Analysis)</h2>
    <div class="stat-grid">
      <div class="stat">
        <div class="label">Total Files</div>
        <div class="value">${n(inv.totalFiles)}</div>
      </div>
      <div class="stat">
        <div class="label">Lines of Code</div>
        <div class="value">${n(inv.totalLines)}</div>
      </div>
      <div class="stat">
        <div class="label">Total Size</div>
        <div class="value">${formatBytes(inv.totalSizeBytes)}</div>
    ...
    <table style="margin-top:1rem">
      <thead><tr><th>Extension</th><th>Files</th><th>Lines</th><th>Size</th></tr></thead>
      <tbody>
        ${topExtensions.map((fc) => `<tr><td>.${fc.extension.toUpperCase()}</td><td>${n(fc.count)}</td><td>${fc.totalLines > 0 ? n(fc.totalLines) : "—"}</td><td>${formatBytes(fc.totalSizeBytes)}</td></tr>`).join("\n        ")}
      </tbody>
    </table>
...
function renderProfileHtml(profile: MigrationProfile): string {
  const confClass = profile.estimate.confidence === "high" ? "badge-high" : "badge-low"
  ...
  <div class="section">
    ...
    <div class="stat-grid">
      <div class="stat">
        <div class="label">Suggested Target</div>
        <div class="value" style="font-size:1.1rem">${profile.suggestedTarget.primary}</div>
        <div class="label">+ ${profile.suggestedTarget.database}</div>
      </div>
      <div class="stat">
        <div class="label">Assessment</div>
        <div class="value" style="font-size:1.1rem">${profile.estimate.assessmentRange}</div>
      </div>
      <div class="stat">
        <div class="label">Migration</div>
        <div class="value" style="font-size:1.1rem">${profile.estimate.migrationRange}</div>
        <div class="label"><span class="badge ${confClass}">${profile.estimate.confidence} confidence</span></div>
      </div>
      <div class="stat">
        <div class="label">Timeline</div>
        <div class="value" style="font-size:1.1rem">${profile.estimate.timelineWeeks} wk</div>
      </div>
    </div>
    <p style="margin-top:1rem;color:#888;font-size:0.85rem">${profile.suggestedTarget.reasoning}</p>
    ${profile.knowledgeGaps.length > 0 ? `
    <h3 style="margin-top:1rem;font-size:0.95rem;color:#888">Knowledge Gaps</h3>
    <ul style="color:#888;font-size:0.85rem;padding-left:1.5rem">
      ${profile.knowledgeGaps.map((g) => `<li>${g}</li>`).join("\n      ")}
    </ul>
    ` : ""}
  </div>`
}
